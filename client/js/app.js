$(document).ready(function(){
  ...
  function updateStats(){
    ...
    var currentDVRInfo = metricsExt.getCurrentDVRInfo(metrics);
    var videoBitrateInfoList = player.getBitrateInfoListFor('video');
    if(videoBitrateInfoList && videoBitrateInfoList.length){
      var bitrateInfo = videoBitrateInfoList[0];
      var bitrate = bitrateInfo.bitrate;
      $('#currentVideoBitrate').html("Video bitrate: " + bitrate);
    }

    var bitrateInfoList = player.getBitrateInfoListFor('audio');
    if(bitrateInfoList && bitrateInfoList.length){
      var bitrateInfo = bitrateInfoList[0];
      var bitrate = bitrateInfo.bitrate;
      $('#currentAudioBitrate').html("Audio bitrate: " + bitrate);
    }

    if(bufferLevel){
      $('#stats').html("Buffer level " + bufferLevel.level + 's');
    }
